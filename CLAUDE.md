...
This is a shared memory substrate where multiple AI agents interact via natural language, with a Neo4j graph database storing knowledge as triples (observations, claims, entities). The system uses a peer-to-peer architecture (v0.3) where every node is identical in networking (HTTP server + WebSocket + HTTP client) and differs only in capabilities (store, LLM, inference, validation, CLI). Nodes discover each other via seed URLs, maintain WebSocket neighbor connections, and propagate state via gossip. No centralized API server or Redis — all coordination is peer-to-peer. The React + D3.js dashboard (port 3000) is now fully connected to the P2P backend via a UI bridge layer (`src/p2p/ui_bridge.py`) mounted on the store node, which translates P2P state into `/v1/` endpoints the frontend expects. The topology panel shows a P2P mesh graph with a status sidebar (per-type node cards, network stats, knowledge counts), the event stream receives live memory events via local listener forwarding, and the knowledge graph renders Neo4j data with color-coded node types. Cross-network connectivity works between Docker containers and localhost via bootstrap URL remapping (`_url_overrides` + `dataclasses.replace` on frozen PeerInfo). Nodes auto-reconnect by re-bootstrapping when all peers die. 61 tests passing (58 P2P + 3 URL override tests). All docs updated to v0.3.
