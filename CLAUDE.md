...
This is a shared memory substrate where multiple AI agents interact via natural language, with a Neo4j graph database storing knowledge as triples — the graph has two layers: an entity graph (Entity-to-Entity edges from observations representing what the system knows) and a provenance graph (Observation/Claim/Resolution nodes tracking where knowledge came from and how confident we are). The system uses a peer-to-peer architecture (v0.3) where every node is identical in networking (HTTP server + WebSocket + HTTP client) and differs only in capabilities (store, LLM, inference, validation, CLI), coordinating via gossip with no centralized server or Redis. The current focus is on clean separation of responsibilities in the knowledge pipeline: `observe()` now only records raw input and builds entity graph structure (no Claim nodes), while claims are exclusively produced by the inference agent through `claim()` — this was a recent refactor that removed duplicate LLM-driven claim creation from the observe path. A new `docs/knowledge_representation.md` documents the full two-layer model, design philosophy, and known limitations (e.g. claims don't yet create entity-to-entity edges, text-based basis matching is heuristic). All docs (`graph_patterns.md`, `neo4j.md`, `design_tracking.md`) have been updated to reflect the new observe/claim separation and fix stale relationship names (`BASED_ON`→`BASIS`, `ABOUT`→`SUBJECT`). 93 tests passing.

**Next up:** Multi-node integration tests, NAT traversal, TLS. See `docs/knowledge_representation.md` for the knowledge model and `docs/visual_ui_design.md` for UI design decisions.
